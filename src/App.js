...
const App = () => {
  ...
  recognition.onresult = async (event) => {
    ...
    if (
      ...
      (command.startsWith("navigate to , go to") && !command.includes("search"))
    ) {
      let pageName;
    ...
    if (
      ...
      history.goForward();
      await speak({ text: "Going to the next page" });
    }

    // stop recogniton and taking commands
    if (command === "stop the voice" || command === "stop taking commands") {
      recognition.stop();
      setStopReco(true);
    ...
    if (
      ...
      await speak({ text: `Scrolling down` });
    } else if (
      command === "scroll up" ||
      command === "scrollup" ||
      command === "go up" ||
      ...
      window.scrollTo(0, 0);
      await speak({ text: `Going to top of the page` });
    } else if (
      ...
      await speak({ text: `Going to bottom of the page` });
    } else if (
      command === "go to half" ||
      command === "gotohalf" ||
      ...
      await speak({ text: `Going to half of the page` });
    } else if (
